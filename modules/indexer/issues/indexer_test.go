...
func searchIssueByID(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expectedIDs: []int64{24, 23, 22, 21, 16, 15, 14, 13, 12, 11, 20, 5, 19, 18, 10, 7, 4, 9, 8, 3, 2},
		},
		{
			opts: SearchOptions{
...
func searchIssueIsClosed(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			[]int64{24, 23, 22, 21, 17, 16, 15, 14, 13, 12, 11, 20, 6, 19, 18, 10, 7, 9, 8, 3, 2, 1},
		},
		{
			SearchOptions{
...
func searchIssueByLabelID(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			[]int64{24, 23, 22, 21, 17, 16, 15, 14, 13, 12, 11, 20, 6, 5, 19, 18, 10, 7, 4, 9, 8, 3},
		},
	}
	for _, test := range tests {
		issueIDs, _, err := SearchIssues(context.TODO(), &test.opts)
...
func searchIssueByTime(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			[]int64{24, 23, 22, 21, 17, 16, 15, 14, 13, 12, 11, 20, 6, 5, 19, 18, 10, 7, 4, 9, 8, 3, 2, 1},
		},
	}
	for _, test := range tests {
		issueIDs, _, err := SearchIssues(context.TODO(), &test.opts)
...
func searchIssueWithOrder(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			[]int64{1, 2, 3, 8, 9, 4, 7, 10, 18, 19, 5, 6, 20, 11, 12, 13, 14, 15, 16, 17, 21, 22, 23, 24},
		},
	}
	for _, test := range tests {
		issueIDs, _, err := SearchIssues(context.TODO(), &test.opts)
