...
class RenameUser {
	 ...
	 *        If derived is false, it is assumed that all shared tables have been updated.
	 */
	...
	private function check(): Status {
		...
		if ( $this->derived ) {
			if ( $oldUser->isRegistered() ) {
				return Status::newFatal( 'renameusererrorexists', $this->oldName );
			} if ( !$newUser->isRegistered() ) {
				return Status::newFatal( 'renameusererrordoesnotexist', $this->newName );
			}
		} else {
			if ( !$oldUser->isRegistered() ) {
				return Status::newFatal( 'renameusererrordoesnotexist', $this->oldName );
			} if ( $newUser->isRegistered() ) {
