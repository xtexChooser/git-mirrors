...
export async function printROA(type: 'json' | 'bird2' | 'bird1' | 'grtr' | 'obgpd', external: boolean, inetFamily: 'ipv4' | 'ipv6' | 'all') {
    let roas = await collectROA(external)
    switch (inetFamily) {
        case 'ipv4': roas = roas.filter(roa => ip.isV4Format(ip.cidr(roa.prefix))); break;
        case 'ipv6': roas = roas.filter(roa => !ip.isV4Format(ip.cidr(roa.prefix))); break;
        case 'all': break;
        default: logger.error({ inetFamily }, 'unknown inet family type')
    }
    switch (type) {
